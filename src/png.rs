...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, &'static str> {
        let target_chunk_type = ChunkType::from_str(chunk_type).unwrap();
        for (i, chunk) in self.chunks.iter().enumerate() {
            if *chunk.chunk_type() == target_chunk_type {
                return Ok(self.chunks.swap_remove(i))
            }
        }
        Err("Found nothing")
    }
...
impl fmt::Display for Png {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        for chunk in self.chunks.iter() {
            f.write_fmt(format_args!("{}", chunk.to_string())).unwrap();
        }
        Ok(())
    }
}
